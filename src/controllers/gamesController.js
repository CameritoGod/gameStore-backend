  ...
  async gameById(req, res) {
    try {
      ...
      res.status(error.response?.status || 500).json({
        message: `Error al obtener el detalle del juego del id ${req.params.id}`,
  ...
  async gameRecommendations(req, res) {
    try {
      const response = await axios.get(BASE_URL, {
        params: {
          key: API_KEY,
          ordering: '-rating',       // mejor rating primero
          page_size: 15,             // pedimos más por seguridad
          ratings_count__gte: 500,   // mínimo votos
          metacritic__gte: 75        // calidad real
        }
      });
    
      const games = response.data.results
        .filter(game => game.background_image) // sin imagen no sirve
        .slice(0, 15) // aseguramos mínimo 10
        .map(game => ({
          id: game.id,
          title: game.name,
          image: game.background_image,
      ...
      res.status(500).json({
        message: 'Error al obtener los juegos recomendados',
        error: error.message
      });
    }
  }
